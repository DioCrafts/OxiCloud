...
impl FolderFsRepository {
    ...
    pub fn new_stub() -> Self {
        ...
        let storage_mediator_stub =
            Arc::new(crate::application::services::storage_mediator::StubStorageMediator::new());
    ...
    async fn count_directory_items(&self, directory_path: &Path) -> FolderRepositoryResult<usize> {
        ...
        let read_dir_result = timeout(read_dir_timeout, read_dir(directory_path)).await;

        match read_dir_result {
            Ok(result) => {
                let mut entries =
                    result.map_err(|e| FolderRepositoryError::StorageError(e.to_string()))?;
                let mut count = 0;
            ...
            Err(_) => Err(FolderRepositoryError::Other(format!(
                "Timeout counting items in directory: {}",
    ...
    pub async fn get_mapped_folder_path(&self, folder_id: &str) -> FolderRepositoryResult<String> {
        let storage_path = self
            .id_mapping_service
            .get_path_by_id(folder_id)
            .await
            .map_err(|e| {
                FolderRepositoryError::StorageError(format!("Failed to get folder path: {}", e))
            })?;
        Ok(storage_path.to_string())
    }

    /// Updates a folder path in the ID mapping service
    pub async fn update_mapped_folder_path(
        &self,
        folder_id: &str,
        new_path: &PathBuf,
        ...
        self.id_mapping_service
            .update_path(folder_id, &storage_path)
            .await
            .map_err(|e| {
                FolderRepositoryError::StorageError(format!("Failed to update folder path: {}", e))
            })
    }
    ...
    pub async fn remove_mapped_folder_id(&self, folder_id: &str) -> FolderRepositoryResult<()> {
        self.id_mapping_service
            ...
            .await
            .map_err(|e| {
                FolderRepositoryError::StorageError(format!("Failed to remove folder ID: {}", e))
            })
    }

    /// Checks if a folder exists at a given storage path
    async fn check_folder_exists_at_storage_path(
        &self,
        storage_path: &StoragePath,
    ) -> FolderRepositoryResult<bool> {
        ...
        tracing::debug!(
            "Checking if folder exists: {} - path: {}",
            exists,
            abs_path.display()
        );
    ...
    async fn create_folder_entity(
        ...
        let folder = if let (Some(created), Some(modified)) = (created_at, modified_at) {
            Folder::with_timestamps(id, name, storage_path, parent_id, created, modified)
        } else {
            Folder::new(id, name, storage_path, parent_id)
        };
        ...
        folder.map_err(|e| match e {
            ...
            FolderError::ValidationError(msg) => FolderRepositoryError::ValidationError(msg),
        })
    }
    ...
    fn from(err: IdMappingError) -> Self {
        match err {
            ...
            IdMappingError::Timeout(msg) => {
                FolderRepositoryError::StorageError(format!("Timeout: {}", msg))
            }
...
impl FolderRepository for FolderFsRepository {
    async fn create_folder(
        ...
        let parent_storage_path = match &parent_id {
            Some(id) => match self._get_folder_storage_path(id).await {
                Ok(path) => {
                    tracing::info!(
                        "Using folder path: {:?} for parent_id: {:?}",
                        path.to_string(),
                        id
                    );
                    Some(path)
        ...
        tracing::info!(
            ...
            folder_storage_path.to_string()
        );

        // Check if folder already exists
        if self
            .check_folder_exists_at_storage_path(&folder_storage_path)
            .await
        ...
        let id = self
            .id_mapping_service
            .get_or_create_id(&folder_storage_path)
            .await
            .map_err(|e| DomainError::internal_error("Folder", e.to_string()))?;
        let folder = self
        ...
        if let Err(e) = &save_result {
            ...
            tracing::info!(
                "Successfully saved ID mapping for folder ID: {} -> path: {} (name: {})",
                id,
                folder_storage_path.to_string(),
                name
            );
    ...
    async fn get_folder(&self, id: &str) -> Result<Folder, DomainError> {
        ...
        if !abs_path.exists() || !abs_path.is_dir() {
            ...
            return Err(DomainError::not_found(
                "Folder",
                format!("Folder {} not found at {}", id, storage_path.to_string()),
            ));
        }

        // Get folder metadata
        let (created_at, modified_at) = self
            .get_folder_metadata(&abs_path)
            .await
            .map_err(DomainError::from)?;
        ...
        let parent_id: Option<String> = if parent.is_none() || parent.as_ref().unwrap().is_empty() {
            ...
            self.id_mapping_service
                .get_or_create_id(parent.as_ref().unwrap())
                .await
                .ok()
        };
    ...
    async fn get_folder_by_path(&self, storage_path: &StoragePath) -> Result<Folder, DomainError> {
        ...
        let id = self
            .id_mapping_service
            .get_or_create_id(storage_path)
            .await?;
        tracing::debug!(
            "Found folder with path: {:?}, assigned ID: {}",
            storage_path.to_string(),
            id
        );
    ...
    async fn list_folders(&self, parent_id: Option<&str>) -> Result<Vec<Folder>, DomainError> {
        ...
        let read_dir_timeout = Duration::from_secs(30);
        let read_dir_result = match timeout(read_dir_timeout, fs::read_dir(&abs_parent_path)).await
        {
            Ok(result) => {
                result.map_err(|e| DomainError::internal_error("Folder", e.to_string()))?
            }
            Err(_) => {
    ...
    async fn list_folders_paginated(
        ...
        tracing::info!(
            "Listing folders in parent_id: {:?} with pagination (offset={}, limit={})",
            parent_id,
            offset,
            limit
        );
        ...
        while let Some(entry_result) = entries.next().await {
            ...
            let folder_result =
                timeout(Duration::from_secs(10), self.get_folder_by_path(&rel_path)).await;
    ...
    async fn rename_folder(&self, id: &str, new_name: String) -> Result<Folder, DomainError> {
        ...
        tracing::debug!(
            "Renaming folder with ID: {}, Name: {}",
            id,
            original_folder.name()
        );

        let renamed_folder = original_folder
            .with_name(new_name)
            .map_err(|e| DomainError::validation_error(e.to_string()))?;

        if self
            .check_folder_exists_at_storage_path(renamed_folder.storage_path())
            .await
            .map_err(DomainError::from)?
        {
            return Err(DomainError::already_exists(
                "Folder",
                renamed_folder.storage_path().to_string(),
            ));
        }
        ...
        self.id_mapping_service
            .update_path(id, renamed_folder.storage_path())
            .await?;
        self.id_mapping_service.save_changes().await?;

        tracing::debug!(
            "Folder renamed successfully: ID={}, New name={}",
            id,
            renamed_folder.name()
        );
        Ok(renamed_folder)
    ...
    async fn move_folder(
        ...
        let target_parent_storage_path = match new_parent_id {
            Some(parent_id) => match self._get_folder_storage_path(parent_id).await {
                Ok(path) => Some(path),
                Err(e) => {
                    return Err(DomainError::internal_error(
                        "Folder",
        ...
        let moved_folder = original_folder
            .with_parent(new_parent_id_option, target_parent_storage_path)
            .map_err(|e| DomainError::validation_error(e.to_string()))?;

        if self
            .check_folder_exists_at_storage_path(moved_folder.storage_path())
            .await
            .map_err(DomainError::from)?
        {
            return Err(DomainError::already_exists(
                ...
                format!(
                    ...
                    moved_folder.storage_path().to_string()
                ),
            ));
        }
        ...
        self.id_mapping_service
            .update_path(id, moved_folder.storage_path())
            .await?;
        self.id_mapping_service.save_changes().await?;

        tracing::debug!(
            "Folder moved successfully: ID={}, New path={:?}",
            id,
            moved_folder.storage_path().to_string()
        );
        Ok(moved_folder)
    ...
    async fn delete_folder(&self, id: &str) -> Result<(), DomainError> {
            ...
            if entry_count > 1000 {
                ...
                tokio::task::spawn_blocking(move || {
                    if let Err(e) = std::fs::remove_dir_all(&path_for_large_removal) {
                        ...
                        return Err(std::io::Error::other(format!(
                            "Failed to remove large directory: {}",
                            e
                        )));
                    }
                    Ok(())
                ...
                .unwrap_or_else(|e| {
                    Err(std::io::Error::other(format!(
                        "Task panicked during directory removal: {}",
                        e
                    )))
                })
            } else {
        ...
        let delete_result = timeout(Duration::from_secs(DELETE_TIMEOUT_SECS), delete_task).await;
        ...
        tracing::info!(
            "Folder deleted successfully: ID={}, Name={}",
            id,
            folder_name
        );
        Ok(())
    ...
    async fn restore_from_trash(
        ...
        self._trash_restore_from_trash(folder_id, original_path)
            .await
            .map_err(DomainError::from)
    }

    async fn delete_folder_permanently(&self, folder_id: &str) -> Result<(), DomainError> {
        self._trash_delete_folder_permanently(folder_id)
            .await
            .map_err(DomainError::from)
    }
}
