...
impl User {
    ...
    pub fn delete(&self) -> bool {
        if let Some(emitter) = &self.emitter {
            emitter.emit(r"\OC\User", "preDelete", &[self]);
        }
        
        let result = self.backend.delete_user(&self.uid);
        
        if let Some(emitter) = &self.emitter {
            emitter.emit(r"\OC\User", "postDelete", &[self]);
        }
    ...
    pub fn set_password(&self, password: &str, recovery_password: &str) -> bool {
        if let Some(emitter) = &self.emitter {
            emitter.emit(r"\OC\User", "preSetPassword", &[self, password, recovery_password]);
        }
        ...
        if self.backend.implements_actions(OC_USER_BACKEND_SET_PASSWORD) {
            ...
            if let Some(emitter) = &self.emitter {
                emitter.emit(r"\OC\User", "postSetPassword", &[self, password, recovery_password]);
            }
