        ...
        self.container.register_service("UserSession", |c| {
            Box::pin(async move {
                ...
                // Set up event listeners
                {
                    ...
                    user_session_clone.listen("\OC\User", "preCreateUser", Box::new(move |args| {
                        Box::pin(async move {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "postCreateUser", Box::new(move |args| {
                        Box::pin(async move {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "preDelete", Box::new(move |args| {
                        Box::pin(async move {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "postDelete", Box::new(move |args| {
                        Box::pin(async move {
                            if let Some(user) = args.get("user") {
                                if let Some(user_obj) = user.downcast_ref::<User>() {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "preSetPassword", Box::new(move |args| {
                        Box::pin(async move {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "postSetPassword", Box::new(move |args| {
                        Box::pin(async move {
                            if let (Some(user), Some(password), Some(recovery_password)) = 
                                (args.get("user"), args.get("password"), args.get("recoveryPassword")) {
                                if let Some(user_obj) = user.downcast_ref::<User>() {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "preLogin", Box::new(move |args| {
                        Box::pin(async move {
                            if let (Some(uid), Some(password)) = (args.get("uid"), args.get("password")) {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "postLogin", Box::new(move |args| {
                        Box::pin(async move {
                            if let (Some(user), Some(password)) = (args.get("user"), args.get("password")) {
                                if let Some(user_obj) = user.downcast_ref::<User>() {
                ...
                {
                    let user_session_clone = Arc::clone(&user_session);
                    user_session_clone.listen("\OC\User", "logout", Box::new(move |_args| {
                        Box::pin(async move {
